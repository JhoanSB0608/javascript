import { getAllClientsByManagerCode, getAllClients } from "./client.js"
...
export const getAllFullNameAndEmailsAndBoss = async () => {
    let res = await fetch("http://172.16.101.146:5532/employee?code_boss=7")
...
export const getAllFullnamePositionDiferentSalesRepresentative = async () => {
    let res = await fetch("http://172.16.101.146:5532/employee?position_ne=Representante Ventas")
    let data = await res.json();
    let dataUpdate = []
    data.forEach(val => {
        if (val.code_boss != null) {
...
export const getAllEmployeeNames = async (code) => {
    let res = await fetch(`http://172.16.101.146:5532/employee?employee_code=${code}`)
    let data = await res.json();
    return data;
}
...
export const getAllEmployeesAndBossesNames = async () => {
    let res = await fetch('http://172.16.101.146:5532/employee').then(res => res.json());
